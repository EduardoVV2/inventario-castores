function MultiSelectTag(el, customs = { shadow: false, rounded: true }) {
    ...
    tagColor.textColor = "#FFFFFF";
    tagColor.borderColor = "#00a886";
    tagColor.bgColor = "#00a886";
    ...
    function createElementInSelectList(option, val, selected = false) {
        ...
        li.innerHTML = "<input type='checkbox' class='input_checkbox'>"; // add the checkbox at the left of the <li>
        li.innerHTML += option.label;
        li.dataset.value = option.value;
        const checkbox = li.firstChild;
        checkbox.dataset.value = option.value;

        // Para búsqueda: ignorar acentos y buscar en cualquier parte del texto
        if (val) {
            const normalizedLabel = option.label.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            const normalizedVal = val.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            if (normalizedLabel.includes(normalizedVal)) {
                ul.appendChild(li);
            }
        } else {
            ul.appendChild(li);
